...
# SELECT REGION. IF WANT TO SELECT ALL REGIONS, COMMENT THE FOLLOWING 2 LINES
...
c.hogar <- c("IV1","IV6","IV8","IV10","IV11","IV12_1","IV12_2","IV12_3","II4_1","II4_2","II4_3","II7","II8",'IV7')
strat <- c("V1","V2","V3","V5","V6","V7","V11","V12","V13","V14","V15","V16","V17", 'V10', 'V22', 'V18')
id <- c('CODUSU','NRO_HOGAR')
num <- c("IX_TOT","IPCF")

vars <-c(id,c.hogar,strat,num)

vars.no.id <- setdiff(vars,c('CODUSU','NRO_HOGAR'))
df<-df[,vars]

# TRANSFORM NUMERICAL VARIABLES TO INTERVALS
# Cantidad de miembros del hogar
df$IX_TOT <- cut(df$IX_TOT, breaks=c(1, 2, 3, 5, 7, 10, 15, Inf), right = FALSE)
# Ingresos per cápita familiar
df$IPCF <- cut(df$IPCF,
                breaks=c(0, 5000, 10000, 30000, 60000, 100000, Inf),
                right = FALSE,
               labels= c('[$0,$5.000)','[$5.000,$10.000)','[$10.000,$30.000)',
                        '[$30.000,$60.000)','[$60.000,$100.000)','[$100.000,Inf)'))


# TRANSFORM ALL VARS TO FACTORS
df[] <- lapply(df[], factor)
...
dfiv <- read_excel("index/INDICE_PREGUNTAS_USU_HOGAR.xlsm")
dfir <- read_excel("index/INDICE_RESPUESTAS_USU_HOGAR.xlsm")
dfa <- df
...
# Change var lists
vars <- unlist(c(dfiv[dfiv$COD %in% vars, "NAME"]), use.names = FALSE)
id <- unlist(c(dfiv[dfiv$COD %in% id, "NAME"]), use.names = FALSE)
vars.no.id <- unlist(c(dfiv[dfiv$COD %in% vars.no.id, "NAME"]), use.names = FALSE)
c.hogar <- unlist(c(dfiv[dfiv$COD %in% c.hogar, "NAME"]), use.names = FALSE)
num <- unlist(c(dfiv[dfiv$COD %in% num, "NAME"]), use.names = FALSE)
strat <- unlist(c(dfiv[dfiv$COD %in% strat, "NAME"]), use.names = FALSE)



# CHECK NANS AND FACTORS WITH ONLY 1 LEVEL/CATEGORY
...
cols_to_remove <- vars.no.id[sapply(df[,vars.no.id], function(x) length(levels(x)) < 2)]
df <- df[, !names(df) %in% cols_to_remove]



# CREATE A TABLE TO SHOW PREPROCESSING RESULT
sample <- df[,names(df) %in% vars.no.id]
sample <- sample[0:30,]
library(gridExtra)
png("result/sample.png", height = 25*nrow(sample), width = 300*ncol(sample), )
grid.table(sample)
dev.off()
...
ggplot(gather(df[,names(df) %in% vars.no.id]), aes(value))+
...
# FIRST PERFORM MCA TO CHECK ACUMULATIVE EXPLAINED VARIANCE
var.mca <- c(c.hogar, strat, num)
df.for.mca<-df[,names(df) %in% var.mca]
...
fviz_screeplot(mca, addlabels = TRUE, ylim = c(0, 20))+theme(
...
# USING IPCF AS THE LABELING VARIABLE
fviz_mca_biplot(mca,
                repel = TRUE,
                label=c("var"),
                axes = c(2,1),
...
ggsave("result/mca/biplot_IPCF.png",width=10,height=15)
...
hcpc <- HCPC(mca, nb.clust = num.of.clust, graph = TRUE)
...
# GRAPH A SUMMARY OF EACH CLUSTER
library(tidyr)
...
for (n in 1:num.of.clust){
    ...
    if(nrow(df.to.sum)<0){
        ...
        image.name=paste(paste("result/cluster/cluster",toString(round(nrow(df.to.sum)/nrow(df.for.mca)*100, digits = 2))),"pc.png")
     ...
     axis.title=element_text(size=14,face="bold"))+
        ...
        #coord_flip()+
        facet_wrap(~key, scales = 'free_x')
        ggsave(image.name,width=20,height=40)
    } 
}
...
var.mca <- c(c.hogar, num)
df.for.mca <- df[,names(df) %in% var.mca]
df.for.mca <- cbind(df.for.mca, Cluster = hcpc$data.clust$clust)

mca <- MCA(df.for.mca, graph = FALSE, ncp=ncp,
           # exclude cluster label as an active mca column, use only for graphs
...
ggsave("result/cluster/biplot_clusters.png",width=16,height=10)
